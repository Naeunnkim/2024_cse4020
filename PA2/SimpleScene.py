...
#추가한 변수들
selected = False #cow selected or not
animating = False #if animating or not
controlPoints = [] #positions of control points
cpCount = -1 #numbers of control points
cowPosition = np.eye(4)
animStartTime = 0
...
def cowFace(currentPos, nextPos):
    global cow2wld

    # direction vector b/w current position and next position
    direction = nextPos - currentPos
    direction = normalize(direction)

    # calculate x axis orthogonal to direction vector
    up = np.array([0, 1, 0])
    xAxis = normalize(np.cross(up, direction))
    yAxis = np.cross(direction, xAxis)

    # rotation matrix
    rotationMatrix = np.eye(4)
    rotationMatrix[:3, 0] = xAxis
    rotationMatrix[:3, 1] = yAxis
    rotationMatrix[:3, 2] = direction

    # apply rotation matrix
    cow2wld[:3, :3] = rotationMatrix[:3, :3]


def display():
    global cameraIndex, cow2wld
    global cowPosition, controlPoints, cpCount, animStartTime, animating, isDrag
    ...
    if animating:
        ...
        if (glfw.get_time() - animStartTime) < 18:
            ...
            if 0 <= t < 1:
                nextcowPosition = spline(aTime, controlPoints[5], controlPoints[0], controlPoints[1], controlPoints[2])
            elif 1 <= t < 2:
                nextcowPosition = spline(aTime, controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3])
            elif 2 <= t < 3:
                nextcowPosition = spline(aTime, controlPoints[1], controlPoints[2], controlPoints[3], controlPoints[4])
            elif 3 <= t < 4:
                nextcowPosition = spline(aTime, controlPoints[2], controlPoints[3], controlPoints[4], controlPoints[5])
            elif 4 <= t < 5:
                nextcowPosition = spline(aTime, controlPoints[3], controlPoints[4], controlPoints[5], controlPoints[0])
            elif 5 <= t < 6:
                nextcowPosition = spline(aTime, controlPoints[4], controlPoints[5], controlPoints[0], controlPoints[1])

            cowFace(getTranslation(cowPosition), getTranslation(nextcowPosition))
            setTranslation(cow2wld, getTranslation(nextcowPosition))
...
def onMouseButton(window,button, state, mods):
    ...
    global selected, animating, controlPoints, cpCount, animStartTime
    GLFW_DOWN=1
    GLFW_UP=0
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox:
                if cpCount < 5:
                    cpCount += 1
                    controlPoints.append(cow2wld.copy()) #현재 소의 위치 복제
                    print("Control Point added at position: ", getTranslation(controlPoints[-1]))
                elif cpCount == 5:
                    animStartTime = glfw.get_time()
                    animating = True
                    print("Animation started with 6 control points")
            print("Left mouse up\n")
